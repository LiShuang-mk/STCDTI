...
class UniProtSeqText(data.ProteinDataset):
    ...
    def load_tsv(self, tsv_file, transform=None, **kwargs):
        with open(tsv_file, "r") as fin:
            ...
            for i, values in enumerate(reader):
                for field, value in zip(
                    fields, values[: len(fields)]
                ):  # to handle rows with unexpected columns
        ...
        self.pad_idx = self.text_tokenizer.convert_tokens_to_ids(
    ...
    def get_item(self, index):
        ...
        if self.text_lazy:
            ...
            if self.seq_lazy:
                attributes = {
                    v: self.texts[v][index] for v in self.text_field2acronym.values()
                }
            else:
                attributes = None
...
class Reaction(data.ProteinDataset):
    ...
    def __init__(self, path, verbose=1, **kwargs):
        ...
        lmdb_files = [
            os.path.join(data_path, "Reaction/Reaction_%s.lmdb" % split)
            for split in self.splits
        ]

        self.load_lmdbs(
            lmdb_files, target_fields=self.target_fields, verbose=verbose, **kwargs
        )
...
class AAV(data.ProteinDataset):
    ...
    def __init__(
        self, path, split="two_vs_many", keep_mutation_region=True, verbose=1, **kwargs
    ):
        path = os.path.expanduser(path)
        ...
        self.load_csv(
            csv_file, target_fields=self.target_fields, verbose=verbose, **kwargs
        )
...
class EnzymeCommission(data.ProteinDataset):
    ...
    def __init__(self, path, branch=None, test_cutoff=0.95, verbose=1, **kwargs):
        ...
        pdb_ids = [
            os.path.basename(pdb_file).split("_")[0] for pdb_file in self.pdb_files
        ]
    ...
    def filter_pdb(self, pdb_ids):
        ...
        pdb_files = []
        data = []
        for sequence, pdb_file, protein in zip(
            self.sequences, self.pdb_files, self.data
        ):
    ...
    def get_item(self, index):
        ...
        item["targets"] = utils.sparse_coo_tensor(
            indices, values, (len(self.tasks),)
        ).to_dense()
        return item
